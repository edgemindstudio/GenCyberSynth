# src/gencysynth/adapters/models/diffusion/base.py
"""
DiffusionAdapterBase.
...
class DiffusionAdapterBase(BaseModelAdapter):
    def __init__(self, variant: str, description: str = "") -> None:
        super().__init__(ModelAdapterSpec(family="diffusion", variant=variant, description=description))

    def _io(self, run_ctx: Any) -> RunIO:
        return self.run_io(run_ctx)

    def checkpoints_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).checkpoints_dir(family="diffusion", variant=self.spec.variant)

    def summaries_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).summaries_dir(family="diffusion", variant=self.spec.variant)

    def synthetic_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).synthetic_dir(family="diffusion", variant=self.spec.variant)
    ...
    def standardize_train_inputs(self, cfg: Dict[str, Any], data: DatasetSplits) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # default diffusion training space: [-1,1]
