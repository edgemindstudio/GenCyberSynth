...
This base defaults to training on [-1,1] (common for diffusion).
"""
...
def cfg_for_hw(x: np.ndarray) -> Dict[str, Any]:
    """
...
class DiffusionAdapterBase(BaseModelAdapter):
    ...
    def standardize_train_inputs(self, cfg: Dict[str, Any], data: DatasetSplits) -> Tuple[np.ndarray, np.ndarray]:
        ...
        x_m11 = to_minus1_1(to_01(x01))  # ensure x01 then map to [-1,1]
        return x_m11, y1h
