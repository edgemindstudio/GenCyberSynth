# src/gencysynth/adapters/models/gaussianmixture/base.py
"""
GaussianMixtureAdapterBase.

GMM conventions
---------------
- Models typically operate on flattened vectors in [0,1] or standardized space.
- Rule A: dataset comes as x01 NHWC; this base provides x_flat for training.

Synthetic artifacts are stored as images in [0,1] NHWC for evaluator.
"""
...
class GaussianMixtureAdapterBase(BaseModelAdapter):
    def __init__(self, variant: str, description: str = "") -> None:
        super().__init__(ModelAdapterSpec(family="gaussianmixture", variant=variant, description=description))

    def _io(self, run_ctx: Any) -> RunIO:
        return self.run_io(run_ctx)

    def checkpoints_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).checkpoints_dir(family="gaussianmixture", variant=self.spec.variant)

    def summaries_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).summaries_dir(family="gaussianmixture", variant=self.spec.variant)

    def synthetic_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).synthetic_dir(family="gaussianmixture", variant=self.spec.variant)
    ...
    def standardize_train_inputs(self, cfg: Dict[str, Any], data: DatasetSplits) -> Tuple[np.ndarray, np.ndarray, Tuple[int, int, int]]:
        ...
        img_shape = tuple(x01.shape[1:])  # (H,W,C)
        x_flat = x01.reshape((x01.shape[0], -1))
        return x_flat, y1h, img_shape  # training uses flat vectors
