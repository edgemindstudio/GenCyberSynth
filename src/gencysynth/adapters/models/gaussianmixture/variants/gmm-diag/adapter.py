# src/gencysynth/adapters/models/gaussianmixture/variants/gmm-diag/adapter.py
...
class GMMDiagAdapter(GaussianMixtureAdapterBase):
    def __init__(self):
        super().__init__(variant="gmm-diag", description="Class-conditional sklearn GaussianMixture (diag cov)")
    ...
    def _train_impl(
        ...
        *,
        ...
        # For GMM we can fit during synth; keep a marker for now.
        ...
        *,
        ...
        for k in range(K):
            ...
            if len(idx) < max(10, n_components):
                # smoke-safe fallback: resample real examples if class too small
                if len(idx) == 0:
                    continue
