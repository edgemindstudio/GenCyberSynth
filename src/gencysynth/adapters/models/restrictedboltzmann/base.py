# src/gencysynth/adapters/models/restrictedboltzmann/base.py
"""
RBMAdapterBase.

RBM conventions
---------------
- Many RBMs generate Bernoulli samples in {0,1}. We store synthetic artifacts in [0,1].
- Training typically uses [0,1] binarized images. This base standardizes x01.
"""
...
class RBMAdapterBase(BaseModelAdapter):
    def __init__(self, variant: str, description: str = "") -> None:
        super().__init__(ModelAdapterSpec(family="restrictedboltzmann", variant=variant, description=description))

    def _io(self, run_ctx: Any) -> RunIO:
        return self.run_io(run_ctx)

    def checkpoints_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).checkpoints_dir(family="restrictedboltzmann", variant=self.spec.variant)

    def summaries_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).summaries_dir(family="restrictedboltzmann", variant=self.spec.variant)

    def synthetic_dir(self, run_ctx: Any) -> Path:
        return self._io(run_ctx).synthetic_dir(family="restrictedboltzmann", variant=self.spec.variant)
