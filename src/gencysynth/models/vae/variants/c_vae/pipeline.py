# src/gencysynth/models/vae/variants/c_vae/pipeline.py
"""
Rule A — Conditional VAE (cVAE) pipeline: training + optional synthesis (.npy).

This module is the *training* side of the c_vae variant. Sampling PNGs for the
unified orchestrator lives in `sample.py` via:
    synth(cfg, output_root, seed) -> manifest

Why this file exists
--------------------
- Build encoder/decoder via variant_local `models.build_models(...)`.
- Train with a custom loop (MSE recon + beta_KL * KL).
- Write *run_scoped*, *dataset_scoped* artifacts (Rule A) using normalized paths:
    artifacts/
      runs/<dataset_id>/vae/c_vae/<run_id>/
        checkpoints/
          E_epoch_XXXX.weights.h5, D_epoch_XXXX.weights.h5
          E_best.weights.h5,       D_best.weights.h5
          E_last.weights.h5,       D_last.weights.h5
        summaries/   (optional)
        synthetic/   (optional: evaluator_friendly .npy dumps)
...
- labels: one_hot (N, K)
- synthesize(): (optional) writes evaluator_friendly .npy in [0, 1]
"""
...
class VAEPipeline:
    ...
    def __init__(self, cfg: Dict[str, Any]):
        # Normalize (Rule A): ensures dataset/run_scoped artifacts paths exist.
        ...
        lr = float(_cfg_get(cfgN, "train.lr", _cfg_get(cfgN, "LR", 2e_4)))
    ...
    def train(
        ...
        *,
        ...
        if y_train.ndim != 2 or y_train.shape[1] != K:
            raise ValueError(f"Expected y_train one_hot shape (N,{K}), got {y_train.shape}")
        ...
        if have_val:
            ...
            if y_val.ndim != 2 or y_val.shape[1] != K:
                raise ValueError(f"Expected y_val one_hot shape (N,{K}), got {y_val.shape}")
        ...
        for epoch in range(1, self.cfg.epochs + 1):
            ...
            # -------------------------
            if have_val:
                improved = val_total < best_val - 1e_6
                if improved:
                    best_val = val_total
                    patience_ctr = 0
                    ...
                    if patience_ctr >= int(self.cfg.early_stopping_patience):
                        print(f"[c_vae] Early stopping at epoch {epoch} (best val={best_val:.4f}).")
                        break
    ...
    # -------------------------------------------------------------------------
    # Optional: evaluator_friendly .npy synthesis under artifacts.synthetic
    # -------------------------------------------------------------------------
    def synthesize(self, decoder: Optional[tf.keras.Model] = None) -> Tuple[np.ndarray, np.ndarray]:
        """
        Generate class_balanced synthetic dataset and write evaluator_friendly .npy files.
        ...
        (x_synth, y_synth)
          ...
          y_synth: one_hot labels,     shape (K*n_pc, K)
        """
        H, W, C = self.cfg.img_shape
        K = self.cfg.num_classes
        ...
        for cls in range(K):
            ...
            # Per_class traceability (matches evaluator expectations)
        ...
        print(f"[c_vae][synthesize] {x_synth.shape[0]} samples ({n_pc}/class) -> {self.cfg.synth_dir}")
