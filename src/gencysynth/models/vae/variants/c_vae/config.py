# src/gencysynth/models/vae/variants/c_vae/config.py
"""
...
Design goals
------------
1) Dataset_aware artifacts (so multiple datasets don't collide):
     {paths.artifacts}/{data.id}/models/vae/c_vae/...
2) Run_aware folders (optional) to keep multiple experiments:
     .../runs/{run.name}/...
3) Backward_compatible aliases:
...
# -----------------------------------------------------------------------------
# Rule A conventions (dataset_aware artifact routing)
# -----------------------------------------------------------------------------
...
class CVAEPaths:
    ...
    Layout (dataset_aware):
      {artifacts_root}/{data_id}/models/vae/c_vae/
        checkpoints/
        summaries/
        synthetic/
        tensorboard/
...
def make_cvae_paths(cfg: Dict[str, Any]) -> CVAEPaths:
    """
    Build dataset_aware (and optionally run_aware) paths for this variant.
    """
    artifacts_root = Path(cfg_get(cfg, "paths.artifacts", "artifacts"))
    data_id = resolve_data_id(cfg)
    run_name = resolve_run_name(cfg)

    # Base: artifacts/<data_id>/models/vae/c_vae
    base = artifacts_root / data_id / "models" / "vae" / "c_vae"
    ...
    tb = artifacts_root / data_id / "tensorboard" / "vae" / "c_vae"
    if run_name:
        tb = tb / run_name
...
def normalize_cfg(cfg_in: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cfg["LR"] = float(cfg.get("LR", cfg_get(cfg, "vae.lr", 2e_4)))
