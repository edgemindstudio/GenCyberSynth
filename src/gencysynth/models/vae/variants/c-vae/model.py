# src/gencysynth/models/vae/variants/c-vae/model.py
"""
Rule A — Conditional VAE (cVAE) model builders.

Important: This file is intentionally *artifact-agnostic*.
...
- Labels are one-hot of length `num_classes`.
- Keras 3: checkpoint filenames must end with `.weights.h5` when using save_weights().

Why no artifacts here?
----------------------
Keeping model-building pure makes the repository scalable across datasets and
...
class Sampling(layers.Layer):
    ...
    def call(self, inputs):
        ...
        # Numerical stability guard: extremely large/small log-var can explode gradients.
...
def _broadcast_labels(img: K.Tensor, onehot: K.Tensor) -> K.Tensor:
    """
    Broadcast labels (B, C) -> spatial label-map (B, H, W, C),
