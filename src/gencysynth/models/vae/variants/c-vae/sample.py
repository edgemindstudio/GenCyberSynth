# src/gencysynth/models/vae/variants/c-vae/sample.py
"""
Rule A — Sampling utilities + unified-CLI synth() entrypoint for Conditional VAE (cVAE).
...
1) Unified Orchestrator (preferred)
   ...
   where `output_root` is already a run-scoped directory like:
       {paths.artifacts}/runs/<dataset>/<vae/c-vae>/<run_id>/png/

   In this mode:
   - We DO NOT invent artifact folders.
   - We ONLY write PNGs under output_root/{class}/{seed}/...
   - We return a JSON-serializable manifest dict.

2) Standalone CLI (optional/dev convenience)
   ----------------------------------------
   python -m gencysynth.models.vae.variants.c_vae.sample --config <yaml>
   This can write `x_synth.npy/y_synth.npy` and per-class dumps under the
   run-scoped synthetic folder from Rule A config normalization.

Key conventions
--------------
- Decoder outputs tanh in [-1, 1]; we rescale to [0, 1] for saving and PNGs.
- Artifacts are dataset-scoped and run-scoped (Rule A):
...
def _build_decoder(
    *,
    ...
    """
    Build decoder via variant-local build_models(...).
...
def _load_decoder_weights(decoder: tf.keras.Model, weights_path: Optional[Path]) -> None:
    ...
    if weights_path is None:
        print("[c-vae][warn] No decoder weights provided; using untrained decoder.")
        return
    if not weights_path.exists():
        print(f"[c-vae][warn] Decoder weights not found at {weights_path}; using untrained decoder.")
        return
    decoder.load_weights(str(weights_path))
    print(f"[c-vae] Loaded decoder weights: {weights_path}")
...
def synth(cfg: dict, output_root: str, seed: int = 42) -> Dict[str, Any]:
    ...
    output_root : str
        Directory where PNGs must be written. The orchestrator supplies a run-scoped
    ...
    Returns
    -------
    manifest : dict (JSON-serializable)
        {
          "dataset": "...",
          "seed": 42,
    ...
    lr = float(_cfg_get(cfgN, "train.lr", _cfg_get(cfgN, "LR", _cfg_get(cfgN, "vae.lr", 2e-4))))
...
def _save_npy_bundle(
    *,
    ...
    """
    Save evaluator-friendly .npy artifacts in a single directory:
...
def _coerce_cfg(cfg_or_argv: Union[Dict[str, Any], list, tuple, None]) -> Dict[str, Any]:
    ...
    p = argparse.ArgumentParser(description="Sample Conditional VAE (c-vae)")
...
def main(argv=None) -> int:
    ...
    Writes .npy artifacts into the run-scoped synthetic directory resolved by
    ...
    ap.add_argument("--write-npy", action="store_true", help="Write evaluator .npy bundle to artifacts.synth_dir")
    ...
    # Where to write .npy bundles (Rule A run-scoped synthetic dir)
    ...
    if args.samples_per_class is not None:
        ...
        if args.balanced:
            ...
            raise ValueError("Use --balanced with --num-samples, or provide exactly one class via --classes.")
    else:
    ...
    print(f"[c-vae] generated x={x_synth.shape} y={y_synth.shape} (standalone)")

    # Optional: write evaluator-friendly npy bundle
    if args.write_npy:
        _save_npy_bundle(synth_dir=synth_dir, x_synth=x_synth, y_synth=y_synth, num_classes=K)
        print(f"[c-vae] wrote .npy bundle -> {synth_dir}")
    ...
    if args.metadata:
        ...
        (synth_dir / "metadata.json").write_text(json.dumps(meta, indent=2), encoding="utf-8")
        print(f"[c-vae] wrote metadata.json -> {synth_dir}")
