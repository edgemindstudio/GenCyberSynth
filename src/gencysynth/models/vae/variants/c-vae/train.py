# src/gencysynth/models/vae/variants/c-vae/train.py
"""
Rule A — Conditional VAE (cVAE) trainer.

This module is a *variant-local* trainer that plugs into the GenCyberSynth
unified orchestration (app/main.py).

What this trainer does
----------------------
1) Normalizes config using Rule A conventions (dataset-scoped run bundle).
...
5) Writes:
   - checkpoints under:  {paths.artifacts}/runs/<dataset>/<vae/c-vae>/<run_id>/checkpoints
   - summaries   under:  {paths.artifacts}/runs/<dataset>/<vae/c-vae>/<run_id>/summaries
   - tensorboard under:  {paths.artifacts}/runs/<dataset>/<vae/c-vae>/<run_id>/summaries/tb
...
- This module accepts both argv-style inputs and dict configs (Rule A pattern).
"""
...
#   src/gencysynth/models/vae/variants/c-vae/
# so imports should be package-relative where possible.
#
...
# Variant-local pipeline + sampler
...
def _set_seed(seed: int) -> None:
    """Best-effort reproducibility across NumPy + TF."""
...
def _run_train(cfg_in: Dict[str, Any]) -> Dict[str, str]:
    """
    Train cVAE and write run-scoped artifacts.
    ...
    # Place TB logs under summaries/tb (variant-scoped, run-scoped)
    tb_dir = _ensure_dir(sums_dir / "tb")

    # ---- Human-friendly run banner ----
    dataset_id = str(_cfg_get(cfg, "data.dataset_id", "dataset"))
    run_id = str(_cfg_get(cfg, "run.run_id", "run"))
    print(f"[c-vae] dataset_id={dataset_id} run_id={run_id} seed={seed}")
    print(f"[c-vae] data_root={data_root} img_shape={img_shape} num_classes={num_classes} latent_dim={latent_dim}")
    print(f"[c-vae] artifacts: ckpt={ckpt_dir} summaries={sums_dir} tb={tb_dir}")
    ...
    try:
        ...
        print(f"[c-vae][warn] encoder/decoder last snapshot save failed: {e}")
    ...
    try:
        ...
        print(f"[c-vae] preview grid → {preview_path}")
    except Exception as e:
        print(f"[c-vae][warn] preview grid failed: {e}")

    # "OK" marker for orchestration sanity checks
    (ckpt_dir / "VAE_LAST_OK").write_text("ok", encoding="utf-8")
...
def _coerce_cfg(cfg_or_argv: Union[Dict[str, Any], list, tuple, None]) -> Dict[str, Any]:
    ...
    # argv parsing
    p = argparse.ArgumentParser(description="Train Conditional VAE (c-vae)")
...
def main(argv=None) -> int:
    """
    argv-style entrypoint so app/main.py can call:
      main(['--config','...'])
