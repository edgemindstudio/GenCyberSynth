...
# • Reproducible degree assignments and per-flow permutations via RANDOM_STATE.
...
# - flatten_images / reshape_to_images: light helpers (model-agnostic)
# =============================================================================
...
class MAFConfig:
    ...
    # Training-time knobs (kept here for convenience; training lives elsewhere)
    LR: float = 2e-4
    CLIP_GRAD: float = 1.0
    PATIENCE: int = 10
...
def flatten_images(
    ...
    -------
    x : np.ndarray
        Either (N, H, W, C) or already-flattened (N, D).
    ...
    - If `assume_01` is True (default), we assume values are already in [0,1].
      If values look byte-like (max > 1.5), we scale by 255 as a best-effort.
    """
    x = np.asarray(x)
    if x.ndim == 4:
        H, W, C = img_shape
    ...
    # Best-effort normalization for legacy byte arrays
...
class MaskedDense(tf.keras.layers.Layer):
    ...
    Why store masks as non-trainable weights?
    ----------------------------------------
    ...
    `trainable=False` weight makes it graph-safe and checkpoint-friendly.
    """
    ...
    def build(self, input_shape):
        ...
        # Store mask as a non-trainable weight so it lives in the right graph
...
class MADE(tf.keras.Model):
    """
    MADE network: masked MLP producing per-dimension (mu, log_sigma).
...
class MAF(tf.keras.Model):
    ...
    def build(self, input_shape):
        ...
        # Warm-build all variables by running once
    ...
    def forward(self, x: tf.Tensor) -> Tuple[tf.Tensor, tf.Tensor]:
        ...
        for i, flow in enumerate(self.flows):
            ...
            # Log-det for diagonal affine transform
            log_det += -tf.reduce_sum(log_sigma, axis=1)

        return z, log_det

    def call(self, x: tf.Tensor) -> tf.Tensor:
        """Keras-friendly call: returns z only."""
        z, _ = self.forward(x)
        return z
    ...
    def _inverse_single_flow(z: tf.Tensor, flow: MADE, input_dim: int) -> tf.Tensor:
        ...
        we can recover x sequentially from i=0..D-1.
        """
        batch = tf.shape(z)[0]
...
def build_maf_model(
    ...
    # Eager-build variables + permutation tensors (safe for checkpoints)
    maf.build((None, D))
    return maf
