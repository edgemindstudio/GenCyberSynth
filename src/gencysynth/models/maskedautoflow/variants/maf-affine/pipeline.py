...
2) Synthesize
   ...
   - Writes evaluator-friendly artifacts:
       gen_class_{k}.npy
       labels_class_{k}.npy
       x_synth.npy, y_synth.npy

Rule A (scalable, dataset-aware artifacts)
-----------------------------------------
All artifacts are dataset-aware and variant-aware:
...
def _one_hot(ids: np.ndarray, num_classes: int) -> np.ndarray:
    """Minimal one-hot encoder for 1-D int labels."""
...
def _resolve_artifact_paths(cfg: Dict[str, Any]) -> tuple[Path, Path, Path]:
    ...
    Subdirs:
      ...
      synthetic/    (write evaluator-friendly .npy dumps)
...
class MAFPipeline:
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        lr = float(_cfg_get(self._raw_cfg, "train.lr", _cfg_get(self._raw_cfg, "LR", 2e-4)))
        ...
        *,
        ...
        for epoch in range(1, self.cfg.EPOCHS + 1):
            ...
            if val_nll is None:
                ...
                improved = (val_nll < best_val - 1e-6)
        ...
        if save_checkpoints:
            (self.cfg.CKPT_DIR / "MAF_LAST_OK").write_text("ok", encoding="utf-8")
    ...
    def synthesize(self) -> tuple[np.ndarray, np.ndarray]:
        """
        Generate a class-balanced synthetic set and write evaluator dumps.

        Returns
        -------
        x_synth : (K*S, H, W, C) float32 in [0,1]
        y_synth : (K*S, K) one-hot
        """
        K = int(self.cfg.NUM_CLASSES)
        ...
        # Safety: drop any non-finite rows (rare, but protects downstream metrics)
        ...
        if not finite.all():
            ...
            # Best-effort top-up to keep exact count
    ...
    def _sample_batch(self, *_, n_per_class: int = 1, **__) -> tuple[np.ndarray, np.ndarray]:
        ...
        Returns (x, y) where y is a dummy one-hot label array for display parity.
        """
        K = int(self.cfg.NUM_CLASSES)
