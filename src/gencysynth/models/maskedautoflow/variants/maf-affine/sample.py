...
- Two output modes:
    1) Unified-CLI `synth(cfg, output_root, seed)`:
         Writes PNGs to {output_root}/{class}/{seed}/... and returns a manifest.
    2) Evaluator-friendly `.npy` dumps (optional helper):
         Writes per-class gen_class_k.npy / labels_class_k.npy + x_synth/y_synth.

Rule A (artifacts) contract
---------------------------
We keep ALL model-owned artifacts dataset-aware:
  ...
  <paths.artifacts>/<data.name>/<model.family>/<model.variant>/
      ...
      synthetic/     (optional: evaluator-friendly .npy dumps, if used)

Unified-CLI output contract
---------------------------
...
# -----------------------------------------------------------------------------
# Optional: evaluator-friendly .npy dumps (NOT used by unified synth output_root)
# -----------------------------------------------------------------------------
...
def synth(cfg: Dict[str, Any], output_root: str, seed: int = 42) -> Dict[str, Any]:
    ...
    2) Resolve Rule-A artifacts for checkpoint + summaries (preview).
    ...
    # ----- Unified-CLI outputs under output_root -----
    ...
    # ----- Optional: evaluator-friendly npy dumps into Rule A synthetic/ -----
