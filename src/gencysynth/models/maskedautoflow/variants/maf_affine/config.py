...
- Exposes helpers to resolve *dataset_scoped* artifact directories
...
def merge_cfg(defaults: Dict[str, Any], user_cfg: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
    ...
    We accept unified cfgs that might store model settings under:
      ...
      - top_level keys (legacy)

    This function keeps it simple:
      - We deep_merge the entire user_cfg onto defaults
...
class ArtifactPaths:
    """
    Dataset_scoped artifact directories for this variant.
...
def resolve_artifacts_root(cfg: Dict[str, Any]) -> Path:
    """
    Resolve the global artifacts root (not dataset_scoped yet).
...
def resolve_paths(cfg: Dict[str, Any]) -> ArtifactPaths:
    """
    Resolve dataset_scoped artifact paths for maf_affine.
