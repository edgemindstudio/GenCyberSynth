...
# Rule A highlights
# -----------------
# Dataset_aware artifacts (scalable across many datasets):
...
#    - writes weights + TB logs only into Rule_A artifact paths
#
# Notes
# -----
# - MAF is unconditional here (labels ignored) and models flattened images.
# - Inputs are float32 in [0,1], shape (N, D).
# - Optional dequantization noise helps continuous likelihood on near_binary inputs.
...
# GPU memory growth (safe no_op on CPU)
...
class TrainConfig:
    ...
    # Dequantization (recommended when inputs are 0/1_ish)
...
def _to_float01(x: np.ndarray) -> np.ndarray:
    """Cast to float32 and map byte_like inputs to [0,1]."""
...
def build_datasets(
    ...
    *,
    ...
    if cfg.DEQUANT_NOISE and cfg.DEQUANT_EPS > 0.0:
        ...
        Xtr = np.clip(Xtr, 1e_6, 1.0 - 1e_6)
        Xva = np.clip(Xva, 1e_6, 1.0 - 1e_6)
...
def train_maf_model(
    ...
    *,
    ...
    """
    Train a MAF by minimizing negative log_likelihood (NLL).
    ...
    (ckpt_dir / "MAF_LAST_OK").write_text("ok", encoding="utf_8")
...
# -----------------------------------------------------------------------------
# Unified_CLI adapter (Rule A + legacy support)
# -----------------------------------------------------------------------------
...
def train(cfg_or_argv) -> Dict[str, Any]:
    """Unified_CLI callable: train(dict) or train(['--config','...'])."""
