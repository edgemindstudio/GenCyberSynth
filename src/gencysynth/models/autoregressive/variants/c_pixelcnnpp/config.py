# src/gencysynth/models/autoregressive/variants/c_pixelcnnpp/config.py

"""
GenCyberSynth — Autoregressive — c_pixelcnnpp — Config
======================================================

Rule A goals
------------
1) Dataset_aware artifacts:
...
This module is intentionally small and dependency_light. It provides helpers to:
- read defaults.yaml
- merge user config
...
def _cfg_get(cfg: Dict[str, Any], dotted: str, default=None):
    """Read nested config keys with dot_notation."""
    cur: Any = cfg
...
def resolve_artifacts(cfg: Dict[str, Any]) -> ArtifactPaths:
    """
    Resolve dataset_aware artifact directories.
    ...
    variant = str(_cfg_get(cfg, "model.variant", "c_pixelcnnpp"))
...
def normalize_legacy_keys(cfg: Dict[str, Any]) -> Dict[str, Any]:
    """
    Provide a flat, legacy_friendly view of the config.
    ...
    out["LR"] = float(_cfg_get(cfg, "train.lr", _cfg_get(cfg, "LR", 2e_4)))
