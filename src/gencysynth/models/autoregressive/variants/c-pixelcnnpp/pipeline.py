# src/gencysynth/models/autoregressive/variants/c-pixelcnnpp/pipeline.py

"""
GenCyberSynth — Autoregressive — c-pixelcnnpp — Pipeline
=======================================================

This file provides a *model-family consistent* train + synth wrapper, matching
...
We want artifacts to scale across:
  ...
  - many variants (c-pixelcnnpp, c-maf-rq, c-ddpm, ...)

Preferred layout (Rule A):
  artifacts/runs/<dataset_id>/<model_tag>/<run_id>/
    checkpoints/     # *.weights.h5 etc.
    synthetic/        # evaluator-friendly .npy dumps
    summaries/        # preview images, small status markers
    tensorboard/      # optional

This pipeline:
  - reads/writes from Rule-A run folders *when run context is present*
  - falls back to legacy artifact folders if run context is absent
  - writes evaluator-friendly per-class .npy dumps to synthetic/
...
Data conventions:
  ...
  - y: (N,K) one-hot float32

Notes
-----
Autoregressive sampling is raster-scan and will be slow compared to GANs.
...
# The builder should live alongside this pipeline (or be re-exported cleanly).
...
def _as_float(x) -> float:
    """Convert scalars / 0-D tensors / arrays to a Python float (for logs)."""
    try:
        return float(x)
    except Exception:
        arr = np.asarray(x)
...
def _safe_slug(x: str) -> str:
    """Filesystem-safe slug for dataset/model/run identifiers."""
    return (
        str(x)
        .strip()
        .replace("\\", "/")
...
def _one_hot_from_ids(ids: np.ndarray, num_classes: int) -> np.ndarray:
    """Convert integer class ids -> one-hot float32."""
...
def _resolve_rule_a_run_dirs(cfg: Dict) -> Optional[Dict[str, Path]]:
    """
    Resolve Rule-A run directories if run context is present.
    ...
    Returns
    -------
    dict with run-root and standard subfolders, or None if any field missing.
    """
    dataset_id = (
...
def _resolve_artifact_dirs(cfg: Dict) -> Dict[str, Path]:
    ...
    Priority:
      1) Rule-A run folders (if dataset/model/run is set)
      2) Explicit per-model ARTIFACTS.* overrides (legacy style)
...
class ARPipelineDefaults:
    ...
    # Model architecture knobs (builder-specific)
    NUM_FILTERS: int = 64
    NUM_LAYERS: int = 4
    NUM_HEADS: int = 4
    FF_MULT: int = 2
...
class ARAutoregressivePipeline:
    ...
    This object is intentionally "small glue" around:
      - a model builder
      - checkpointing
      - evaluator-friendly artifact dumps
    """
    ...
    def train(
        ...
        """
        Fit the AR model and write checkpoints using Rule-A directories.

        Expected inputs:
          x_train: float32 (N,H,W,C) in [0,1]
          y_train: float32 (N,K) one-hot
        """
        ...
        if y_train.ndim != 2 or y_train.shape[1] != self.num_classes:
            raise ValueError(f"y_train must be one-hot (N,{self.num_classes}); got {y_train.shape}")
        ...
        if has_val:
            ...
            if y_val.ndim != 2 or y_val.shape[1] != self.num_classes:
                raise ValueError(f"y_val must be one-hot (N,{self.num_classes}); got {y_val.shape}")
    ...
    def synthesize(self, model: Optional[tf.keras.Model] = None) -> tuple[np.ndarray, np.ndarray]:
        """
        Generate a class-balanced synthetic dataset via raster-scan sampling.

        If `model` is None:
          - build a fresh model
          - load best/last/latest-epoch checkpoint from self.ckpt_dir if present

        Writes evaluator-friendly artifacts to:
          <run>/synthetic/
        """
        if model is None:
        ...
        # Generate class-by-class to keep memory bounded and filenames deterministic.
        for cls in range(self.num_classes):
            ...
            # Evaluator contract: per-class dumps.
        ...
        try:
            ...
            # Non-fatal: evaluator only needs the .npy files.
            pass
    ...
    def _sample_autoregressive(
        ...
        """
        Pixel-wise raster scan sampling (vectorized over batch).

        The model predicts per-pixel probabilities p(x_ij | x_<ij, y).
        ...
        # Raster order: top-left -> bottom-right.
        for i in range(H):
            for j in range(W):
                ...
                # Bernoulli sampling (channel-wise).
...
# Back-compat alias (some code imports AutoregressivePipeline)
