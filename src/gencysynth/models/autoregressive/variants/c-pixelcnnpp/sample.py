# src/gencysynth/models/autoregressive/variants/c-pixelcnnpp/sample.py

"""
GenCyberSynth — Autoregressive — c-pixelcnnpp — Sampling + Unified CLI synth
============================================================================

This module provides:
  - load_ar_from_checkpoints(...): build model + load weights (best→last)
  - save_grid_from_ar(...): quick preview PNG grid
  - synth(cfg, output_root, seed): unified-CLI entrypoint that writes PNGs + manifest
...
This sampler:
- *reads checkpoints* from Rule-A run directories when run context is provided
- falls back to legacy checkpoint locations when run context is not provided

Back-compat: if Rule A keys are missing, we use:
...
Notes
-----
- Raster-scan sampling is sequential over pixels and therefore slow; that's OK for
...
def _safe_slug(x: str) -> str:
    """Filesystem-safe slug for run ids / tags."""
    return (
        str(x)
        .strip()
        .replace("\\", "/")
...
def _resolve_checkpoint_dir(cfg: Dict) -> Path:
    """
    Prefer Rule-A checkpoints if available; else fall back to legacy locations.
    """
...
def _sample_autoregressive(
    ...
    *,
    ...
    For binary-ish pixels, we sample Bernoulli(u < p).
    """
    if seed is not None:
    ...
    for i in range(H):
        for j in range(W):
            # Model outputs per-pixel probs in [0,1]
...
def synth(cfg: dict, output_root: str, seed: int = 42) -> Dict:
    ...
    Reads checkpoints from:
      - Rule-A run checkpoints if dataset/model/run context provided
    ...
    model_tag = _cfg_get(cfg, "model.tag", None) or cfg.get("model_tag", "autoregressive/c-pixelcnnpp")
