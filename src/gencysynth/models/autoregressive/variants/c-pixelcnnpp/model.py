# src/gencysynth/models/autoregressive/variants/c-pixelcnnpp/model.py

"""
GenCyberSynth — Autoregressive — c-pixelcnnpp — Model
=====================================================

This module defines the *model-only* portion of the conditional PixelCNN-style
...
-------------------------
- MaskedConv2D: PixelCNN-style masked convolution layer (Mask A/B).
- build_conditional_pixelcnn(...): Keras model returning per-pixel probabilities.

Conventions
-----------
- Images are channels-last: (H, W, C)
- Inputs x are expected in [0,1] for training with pixelwise BCE.
- Conditioning is class-based: y is one-hot (num_classes,)
...
class MaskedConv2D(layers.Layer):
    """
    PixelCNN-style masked conv using explicit kernel/bias weights.
    ...
    def call(self, inputs):
        # Apply the causal mask on-the-fly.
...
def build_conditional_pixelcnn(
    ...
    *,
    ...
    Architecture
    ------------
    - Input: image x (H,W,C), one-hot label y (K,)
    ...
    ----------
    img_shape : (H,W,C)
    num_classes : number of classes (one-hot length)
    filters : hidden channels in the PixelCNN stack
    masked_layers : number of Mask-B layers
    ...
    - This model returns probabilities, not logits. Training should use BCE with
      from_logits=False.
    - For multi-channel inputs, set out_channels=C (default) to predict each
    ...
    y_in = layers.Input(shape=(K,), name="onehot")              # one-hot label
