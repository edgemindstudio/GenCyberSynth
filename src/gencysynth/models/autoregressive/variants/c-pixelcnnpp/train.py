# src/gencysynth/models/autoregressive/variants/c-pixelcnnpp/train.py

"""
GenCyberSynth — Autoregressive — c-pixelcnnpp — Training
=======================================================

This module trains a **conditional autoregressive** (PixelCNN++) model in a
small, explicit, framework-idiomatic way:
...
Back-compat:
- If a Rule-A run context is NOT provided, we fall back to legacy locations:
...
def _safe_slug(x: str) -> str:
    """
    Convert arbitrary strings (e.g., model tags with slashes) into a filesystem-safe slug.
...
def _resolve_rule_a_dirs(cfg: Dict) -> Optional[Dict[str, Path]]:
    """
    If the config includes a Rule-A run context, return canonical dirs.
...
def make_datasets(
    ...
    Inputs are assumed already normalized to [0,1] and labels are one-hot.
    """
...
def _train_from_cfg(cfg: Dict) -> None:
    """
    Orchestrator-facing training entrypoint.
    ...
    if rule_a_hint is not None:
        print(f"[rule-a] dataset={_cfg_get(cfg,'dataset.id')} | model={_cfg_get(cfg,'model.tag')} | run={_cfg_get(cfg,'run.id')}")
        print(f"[rule-a] root={Path(rule_a_hint['root']).resolve()}")
...
def train(cfg_or_argv) -> int:
    """
    Unified-CLI entrypoint. Returns rc=0 on success (or raises on error).
    """
