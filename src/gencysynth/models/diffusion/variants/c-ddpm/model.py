# src/gencysynth/models/diffusion/variants/c-ddpm/model.py
"""
GenCyberSynth — Diffusion family — c-DDPM variant (Conditional) — Model Builders
===============================================================================

RULE A (Scalable artifact policy)
---------------------------------
This file is a **pure model-construction module**.
...
All I/O (checkpoints, samples, logs) belongs to:
  - variants/c-ddpm/train.py   (writes run-scoped checkpoints + tensorboard)
  - variants/c-ddpm/samply.py  (reads run-scoped checkpoints, writes run-scoped samples + manifest)
...
- build_diffusion_model(...): compact conditional UNet-like εθ predictor
...
class SinusoidalTimeEmbedding(layers.Layer):
    ...
    -----
    t : Tensor
        int32/float32 tensor of shape (B,) or (B,1) or scalar per-batch.
    ...
    ----------------------
    - Uses sin/cos pairs for an even-sized embedding (2 * half_dim).
    - If dim is odd, uses a Dense projection to reach exact dim.
    - Handles tiny dims safely (avoids divide-by-zero when half_dim == 1).
    """
...
def _broadcast_to_spatial(emb: tf.Tensor, h: int, w: int, name_prefix: str = "cond") -> tf.Tensor:
    ...
    We wrap the ops in Lambda layers to keep everything KerasTensor-safe.
...
def build_diffusion_model(
    *,
    ...
    - Model is compiled with Adam + MSE (standard ε-prediction objective).
    ...
    # -----------------------------------------------------------------
    # Compile (standard ε-prediction objective)
    # -----------------------------------------------------------------
    model.compile(
