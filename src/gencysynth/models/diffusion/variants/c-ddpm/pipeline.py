# src/gencysynth/models/diffusion/variants/c-ddpm/pipeline.py
"""
GenCyberSynth — Diffusion family — c-DDPM variant (Conditional) — Pipeline
=========================================================================
...
have:
  - variants/c-ddpm/train.py   : training loop + checkpoints/logging
  - variants/c-ddpm/samply.py  : sampling + manifest writing
...
----------------------------
- invent non-scalable paths like artifacts/diffusion/... (dataset-less)
...
- Checkpoints use Keras 3-friendly "*.weights.h5".

This pipeline keeps the old "class" interface but now it is Rule-A compliant.
"""
...
# -----------------------------------------------------------------------------
# Local model builder (kept import-safe)
# -----------------------------------------------------------------------------
...
# -----------------------------------------------------------------------------
from .train import build_alpha_hat, train_diffusion  # Rule-A updates live in train.py
from .samply import sample_batch  # Rule-A updates live in samply.py
...
def _enable_gpu_mem_growth() -> None:
    """Avoid TF reserving all VRAM on shared GPUs (HPC-friendly)."""
...
# =============================================================================
# DiffusionPipeline (Rule-A compliant)
# =============================================================================
class DiffusionPipeline:
    ...
    IMPORTANT:
    This class is now Rule-A compliant: it always resolves a RunContext and
    ...
    DEFAULTS: Dict[str, Any] = {
        ...
        "DIFFUSION_STEPS": 200,     # reverse sampling steps (preview-friendly)
    }
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        # Canonical run-scoped directories
        ...
        # Build model (fresh init; weights will be saved/loaded from run-scoped ckpts)
        ...
        self.logger.info("=== c-DDPM PIPELINE INIT ===")
    ...
    def train(self) -> tf.keras.Model:
        ...
        # -----------------------------
        # Noise schedule (alpha-hat) used by training objective
        # -----------------------------
    ...
    def synthesize(self, model: Optional[tf.keras.Model] = None) -> SampleResult:
        """
        Generate class-balanced PNGs under:
        ...
        - For very large synthesis (millions), prefer a dedicated sampling script
          + sharding; this function is meant for "run-sized" generation.
        """
        if model is None:
    ...
    def _build_and_load_for_sampling(self) -> tf.keras.Model:
        """
        Build a fresh model with the same architecture and load run-scoped weights.
        """
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(description=f"c-DDPM Pipeline ({MODEL_TAG})")
    ...
    parser.add_argument("--no-sample", action="store_true", help="Skip sampling")
