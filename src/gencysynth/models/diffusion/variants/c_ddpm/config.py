# src/gencysynth/models/diffusion/variants/c_ddpm/config.py
"""
GenCyberSynth — Diffusion family — c_DDPM variant — Config helpers
=================================================================

RULE A (Scalable artifact policy)
---------------------------------
This module is **config_only**:
...
class CDdpmParams:
    """
    Canonical c_DDPM parameters resolved from config.
...
def resolve_params(cfg: Mapping[str, Any]) -> CDdpmParams:
    """
    Resolve parameters using the repo_wide conventions.
    ...
    lr = float(cfg_get(cfg, "LR", cfg_get(cfg, "diffusion.lr", 2e_4)))
    ...
    beta_start = float(cfg_get(cfg, "BETA_START", cfg_get(cfg, "diffusion.beta_start", 1e_4)))
    beta_end = float(cfg_get(cfg, "BETA_END", cfg_get(cfg, "diffusion.beta_end", 2e_2)))
