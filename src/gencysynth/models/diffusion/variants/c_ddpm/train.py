# src/gencysynth/models/diffusion/variants/c_ddpm/train.py
"""
GenCyberSynth — Diffusion family — c_DDPM variant (Conditional) — Training
========================================================================
...
This module MUST NOT invent ad_hoc directories like:
...
So *every* read/write must be dataset_aware and run_aware.
...
- Builds the c_DDPM model (via diffusion.models.build_diffusion_model).
- Trains ε_prediction objective with early stopping.
- Writes checkpoints + tensorboard logs under the run directory.

What this module does NOT do
----------------------------
- Sampling/synthesis (belongs in variants/c_ddpm/sample.py)
- Evaluation metrics/reporting (belongs under gencysynth/eval or reporting/)
- Any global artifact layouts outside Rule A

CLI usage
---------
python -m gencysynth.models.diffusion.variants.c_ddpm.train --config configs/<...>.yaml
...
# NOTE: model builder imported lazily inside _train_from_cfg to keep import_safe.
...
def _set_seeds(seed: int) -> None:
    """Best_effort determinism for numpy + TF."""
...
def build_alpha_hat(
    T: int,
    beta_start: float = 1e_4,
    beta_end: float = 2e_2,
    schedule: str = "linear",
) -> np.ndarray:
    """
    Build ᾱ (alpha_hat) schedule used by DDPM_style training.
    ...
    if schedule == "cosine":
        ...
        alpha_hat = np.clip(alpha_hat, 1e_5, 1.0)
...
def train_step(
    ...
    y_onehot: tf.Tensor,          # one_hot labels, shape (B,K)
    t_vec: tf.Tensor,             # integer timesteps, shape (B,)
    alpha_hat_tf: tf.Tensor,      # ᾱ_t tensor, shape (T,)
    optimizer: tf.keras.optimizers.Optimizer,
) -> tf.Tensor:
    """
    One optimization step for ε_prediction objective.
...
def eval_step(
    ...
    """Forward_only ε_prediction loss (no gradients)."""
    noise = tf.random.normal(shape=tf.shape(x0))
...
def train_diffusion(
    *,
    ...
    """
    Train c_DDPM with ε_prediction objective, early stopping, and checkpoints.

    IMPORTANT:
    - This function is intentionally "path_agnostic" except for ckpt_dir,
    ...
    # Sample random t per batch (tf.function_safe)
...
def _coerce_cfg(cfg_or_argv) -> Dict[str, Any]:
    ...
    Accept either:
      - dict config
      - argv_like list/tuple containing optional: --config <path>
...
def _train_from_cfg(cfg: Dict[str, Any]) -> Dict[str, float]:
    """
    End_to_end training driver that enforces Rule A.

    Responsibilities:
    - ensure cfg has stable model identity
    - resolve run context (dataset_id, model_tag, run_id, seed)
    - create run/log directories (dataset_aware and run_aware)
    ...
    # Run_scoped logger (writes to artifacts/logs/<dataset_id>/<model_tag>/<run_id>/run.log)
    logger = get_run_logger(name=f"{MODEL_TAG}:{ctx.run_id}", log_dir=log_dir)

    logger.info("=== c_DDPM TRAIN START ===")
    ...
    lr = float(_cfg_get(cfg, "LR", _cfg_get(cfg, "diffusion.lr", 2e_4)))
    ...
    beta_start = float(_cfg_get(cfg, "BETA_START", _cfg_get(cfg, "diffusion.beta_start", 1e_4)))
    beta_end = float(_cfg_get(cfg, "BETA_END", _cfg_get(cfg, "diffusion.beta_end", 2e_2)))
    ...
    # -----------------------------
    # Data loading (shared loader returns images in [0,1] + one_hot labels)
    # -----------------------------
    ...
    # -----------------------------
    # Build model (local import keeps module import_safe for tooling)
    # -----------------------------
...
def train(cfg_or_argv) -> Dict[str, float]:
    ...
    Accepts:
      - argv_like list/tuple  -> will parse YAML from --config
