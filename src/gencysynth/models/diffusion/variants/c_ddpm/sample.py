# src/gencysynth/models/diffusion/variants/c_ddpm/samply.py
"""
GenCyberSynth — Diffusion family — c_DDPM variant (Conditional) — Sampling / Synthesis
====================================================================================
...
- synth(cfg, output_root, seed) -> dict
    Backward_compatible wrapper: resolves a RunContext then calls sample(...)
...
# so this module remains import_safe for CLI tooling.
...
# =============================================================================
# Diffusion schedule + reverse sampling (preview_friendly)
# =============================================================================
def _linear_alpha_hat_schedule(
    ...
    """
    Linear beta schedule; returns ᾱ_t = ∏_{s<=t} (1 - β_s) for t=0..T_1.
    """
...
def _reverse_diffuse(
    ...
    *,
    ...
    """
    Perform a DDPM_style reverse diffusion pass given class_condition labels.
...
def save_grid_from_model(
    ...
    *,
    ...
    Canonical per_class PNG outputs are produced by sample(...).
    """
...
def sample(cfg: Dict[str, Any], ctx: RunContext) -> SampleResult:
    ...
    logger.info("=== c_DDPM SAMPLE START ===")
    ...
    # Load model from run_scoped checkpoints (preferred)
    ...
    # -----------------------------
    # Generate and write per_class images
    # -----------------------------
    per_class_counts: Dict[str, int] = {str(k): 0 for k in range(K)}
    paths: List[Dict[str, Any]] = []

    for k in range(K):
        # Per_class seed offset: deterministic but different across classes
...
def synth(cfg: Dict[str, Any], output_root: str, seed: int = 42) -> Dict[str, Any]:
    ...
    - injects model.tag = diffusion/c_ddpm
