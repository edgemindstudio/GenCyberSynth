# src/gencysynth/models/gaussianmixture/variants/c-gmm-full/train.py
"""
GenCyberSynth — GaussianMixture family — c-gmm-full variant (per-class, full covariance) — Training
===============================================================================================
...
- Trains one full-covariance GaussianMixture model per class.
...
CLI entry
---------
python -m gencysynth.models.gaussianmixture.variants.c-gmm-full.train --config <path/to/config.yaml>
"""
...
def _save_preview_grid(
    *,
    ...
    """
    Sample a small number of images from each fitted per-class GMM checkpoint,
    ...
    # Load global fallback if present (used when a per-class model is missing)
...
# =============================================================================
# Core training: per-class GMMs (full covariance)
# =============================================================================
def train_per_class_gmms(
    *,
    ...
    for k in range(num_classes):
        ...
        if count < 2:
            warn = f"[warn] Class {k} has <2 samples; skipping per-class GMM (will rely on fallback at sampling)."
            print(warn)
            if logger:
                logger.warning(warn)
            continue
...
def run_from_file(cfg_path: Path) -> int:
    ...
    logger.info("=== c-gmm-full TRAIN START ===")
    ...
    reg_covar = float(_cfg_get(cfg, "GMM_REG_COVAR", 1e-6))
    ...
    # -------------------------------------------------------------------------
    # Train per-class GMMs and save under run checkpoints
    # -------------------------------------------------------------------------
    ...
    logger.info("=== c-gmm-full TRAIN END ===")
    return 0
...
def main(argv: Optional[list[str]] = None) -> int:
    p = argparse.ArgumentParser(description=f"Train per-class GMMs ({MODEL_TAG})")
