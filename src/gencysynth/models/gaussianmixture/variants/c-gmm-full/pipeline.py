# src/gencysynth/models/gaussianmixture/variants/c-gmm-full/pipeline.py
"""
GenCyberSynth — GaussianMixture — c-gmm-full — Train + Synthesize Pipeline
========================================================================
...
- Training class-conditional GMMs (one per class), with optional global fallback.
- Optional PCA front-end (fit once, train GMMs in PCA space).
...
# Rule-A paths (preferred, used by resolve_run_context):
paths:
  artifacts: artifacts

model:
  tag: gaussianmixture/c-gmm-full
...
class GaussianMixturePipeline:
    """
    Orchestrates training + synthesis for class-conditional GMMs under Rule A.
    ...
    def __init__(self, cfg: Dict[str, Any], ctx: RunContext):
        ...
        self.cfg = GMMPipelineConfig(
            ...
            TOL=_resolve_float(self.cfg_raw, "TOL", 1e-3),
            REG_COVAR=_resolve_float(self.cfg_raw, "REG_COVAR", 1e-6),
    ...
    def _fit_gmm_with_retries(
        ...
        *,
        ...
        Why retries:
        - Full-covariance GMM can fail on ill-conditioned data (especially image
          pixels or low-sample classes).
    ...
    def synthesize(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Drop non-finite samples (rare but safe)
        mask = np.isfinite(x_synth).all(axis=(1, 2, 3))
        if not mask.all():
            dropped = int((~mask).sum())
            self.logger.warning(f"Dropping {dropped} non-finite synthetic samples.")
...
# -----------------------------------------------------------------------------
# Rule-A adapters (so orchestrator can call train(cfg, ctx) / synth(cfg, ctx))
# -----------------------------------------------------------------------------
...
def synth(cfg: Dict[str, Any], ctx: RunContext) -> int:
    ...
    and write evaluator-ready .npy dumps to ctx.run_dir/synthetic.
...
# -----------------------------------------------------------------------------
# Legacy-friendly wrapper (if old code instantiates pipeline without ctx)
# -----------------------------------------------------------------------------
def build_pipeline(cfg: Dict[str, Any]) -> GaussianMixturePipeline:
    """
    Convenience builder that resolves a Rule-A RunContext from config.
