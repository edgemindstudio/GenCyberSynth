# src/gencysynth/models/gaussianmixture/variants/c-gmm-full/samply.py
"""
GenCyberSynth — GaussianMixture — c-gmm-full — Sampling / Synthesis
==================================================================
...
def sample_balanced_from_models(
    ...
    *,
    ...
    for k in range(K):
        ...
        # Ensure deterministic per-class but non-identical streams:
...
def sample(cfg: Dict[str, Any], ctx: RunContext) -> SampleResult:
    ...
    logger.info("=== c-gmm-full SAMPLE START ===")
    ...
    # Generate balanced batch in-memory
    ...
    # Write per-class PNGs and manifest paths (relative to run_dir)
    ...
    logger.info("=== c-gmm-full SAMPLE END ===")
...
def synth(cfg: Dict[str, Any], output_root: str, seed: int = 42) -> Dict[str, Any]:
    ...
    - injects model.tag = gaussianmixture/c-gmm-full
