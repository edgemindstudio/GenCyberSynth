# src/gencysynth/models/gaussianmixture/variants/c_gmm_full/config.py
"""
GenCyberSynth — GaussianMixture — c_gmm_full — Config & Defaults
===============================================================

This module defines **variant_scoped** defaults and small helpers used by:
...
# -----------------------------------------------------------------------------
# Dotted_key config helper (shared pattern across variants)
# -----------------------------------------------------------------------------
...
class CfgDefaults:
    ...
    # Run meta
    seed: int = 42
    dataset_id: str = "USTC_TFC2016_40x40_gray"
...
def normalize_cfg(cfg_in: Dict[str, Any]) -> Dict[str, Any]:
    """
    Return a shallow_normalized cfg dict.

    Goals:
    - Provide **canonical keys** used by Rule_A_aware orchestrators:
        cfg["model"]["tag"]
        cfg["run_meta"]["seed"]
    ...
    if isinstance(cfg["gmm"], dict):
        ...
        # Optional per_class override maps (kept as dict[int,int] downstream)
