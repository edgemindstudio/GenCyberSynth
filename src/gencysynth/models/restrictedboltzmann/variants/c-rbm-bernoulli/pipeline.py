# src/gencysynth/models/restrictedboltzmann/variants/c-rbm-bernoulli/pipeline.py
"""
...
You may override specific artifact dirs via:
  cfg['artifacts']['restrictedboltzmann']['c-rbm-bernoulli']['checkpoints'|'synthetic'|'summaries']

This module is designed to be called by the unified orchestrator (app/main.py),
and to produce evaluator-friendly .npy outputs (not just PNGs).

Notes
-----
- RBM is trained per-class (one RBM per label).
...
#   src/gencysynth/models/restrictedboltzmann/variants/c-rbm-bernoulli/
...
# This avoids duplicating dataset-id parsing logic and keeps paths consistent
...
def _labels_to_int(y: np.ndarray, num_classes: int) -> np.ndarray:
    """
    Accept one-hot (N,K) or integer (N,) labels and return int labels (N,).
    """
    y = np.asarray(y)
...
def _one_hot(ids: np.ndarray, num_classes: int) -> np.ndarray:
    """Convert integer labels -> one-hot float32."""
...
class RBMPipeline:
    ...
    • Synthesize class-balanced samples and write evaluator-friendly .npy files.
    • Keep all artifacts dataset-scoped under Rule A.
    """
    ...
    def __init__(self, cfg: Dict[str, Any]):
        ...
        self.cfg = RBMPipelineConfig(
            ...
            LR=float(cfg.get("LR", cfg.get("RBM_LR", 1e-3))),
        ...
        # ---- Rule A artifacts ----
        # We delegate dataset-id resolution and override handling to sample.py
        ...
        # In-memory RBM handles (one per class). We still load from disk for synth.
    ...
    def _class_ckpt_dir(self, class_id: int) -> Path:
        """
        Per-class checkpoint folder under Rule A:
        ...
        for epoch in range(1, int(self.cfg.EPOCHS) + 1):
            ...
            for v0 in train_ds:
                if str(self.cfg.TRAIN_MODE).lower() == "cd":
                    ...
                    # mse: use optimizer
                    loss = rbm.train_step_mse(v0, optimizer=optimizer, k=int(self.cfg.CD_K))  # type: ignore[arg-type]
            ...
            if val_ds is not None:
                ...
                # If no val set provided, use train loss as best-effort monitor.
                val_loss = train_loss
    ...
    def train(
        ...
        y_train : (N,) ints or (N,K) one-hot
        x_val/y_val : optional explicit validation data (used per-class)
        ...
        self._log(
            "train",
            f"Training RBMs (per-class) on {len(X)} samples | dim={V} | classes={K} "
        ...
        for k in range(K):
            ...
            # Optional per-class val set if explicit val provided
            Xk_val = None
        ...
        (self.paths.checkpoints / "RBM_LAST_OK").write_text("ok", encoding="utf-8")

        # Optional: save a small training preview grid (best-effort)
        try:
    ...
    def synthesize(self) -> Tuple[np.ndarray, np.ndarray]:
        """
        Generate a class-balanced synthetic dataset and write evaluator files.
        ...
        y_synth : float32, shape (N_total, K), one-hot labels
        """
        H, W, C = self.cfg.IMG_SHAPE
        K = int(self.cfg.NUM_CLASSES)
        ...
        for k in range(K):
            ...
            # Per-class dumps (evaluator contract used across families)
        ...
        # Optional: synth preview grid (best-effort)
        try:
