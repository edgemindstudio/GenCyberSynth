# src/gencysynth/models/restrictedboltzmann/variants/c-rbm-bernoulli/config.py
"""
Rule A config normalizer for: restrictedboltzmann/variants/c-rbm-bernoulli
...
- Resolve Rule A artifact paths so pipelines/sample/train write to the
  correct run-scoped folders even when multiple datasets are used.
...
def _resolve_scope(cfg: Mapping[str, Any]) -> RunScope:
    ...
    name = str(_cfg_get(cfg, "variant.name", "c-rbm-bernoulli"))
