# src/gencysynth/models/restrictedboltzmann/variants/c-rbm-bernoulli/sample.py
"""
Sampling utilities for the Bernoulli–Bernoulli RBM (c-rbm-bernoulli).
...
- synthesize_to_artifacts(cfg, ...)  -> writes evaluator-friendly .npy files (Rule A)
...
class ArtifactPaths:
    ...
    Default layout:
      artifacts/<dataset_id>/restrictedboltzmann/c-rbm-bernoulli/{checkpoints,synthetic,summaries}
    """
    root: Path
    checkpoints: Path
    synthetic: Path
...
def _resolve_artifact_paths(cfg: Dict[str, Any]) -> ArtifactPaths:
    ...
    syn_override = _cfg_get(cfg, "artifacts.restrictedboltzmann.c-rbm-bernoulli.synthetic", None)
...
def sample_gibbs(
    ...
    *,
    ...
    k : int
        Number of Gibbs steps to run after burn-in.
    burn_in : int
        Number of Gibbs steps to run before sampling.
    init : optional
        Initial visible state (num_samples, V) or image-shaped (num_samples,H,W,C).
...
def _load_rbm(ckpt_path: Path, visible_dim: int, hidden_dim: int) -> BernoulliRBM:
    """
    Instantiate an RBM and load weights from a Keras-compatible weights file.
...
# =============================================================================
# Evaluator-friendly artifact writing (Rule A)
# =============================================================================
...
def synthesize_to_artifacts(
    ...
    *,
    ...
    """
    Rule A synthesis:
      - Loads per-class RBM checkpoints from dataset-scoped ckpt root
      - Samples S samples per class (Gibbs)
      - Writes evaluator-friendly .npy files into dataset-scoped synthetic dir:
          gen_class_<k>.npy, labels_class_<k>.npy, x_synth.npy, y_synth.npy

    Returns
    -------
    x_synth : float32 (N,H,W,C) in {0,1}
    y_synth : float32 (N,K) one-hot
    """
