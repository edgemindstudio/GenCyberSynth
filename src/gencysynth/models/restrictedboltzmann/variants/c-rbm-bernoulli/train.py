# src/gencysynth/models/restrictedboltzmann/variants/c-rbm-bernoulli/train.py
"""
Bernoulli–Bernoulli Restricted Boltzmann Machine (RBM) training (per-class) for GenCyberSynth.

Rule A (Artifact + Dataset Scoping)
-----------------------------------
This variant MUST read/write artifacts in a dataset-scoped, variant-scoped layout:

  artifacts/<dataset_id>/restrictedboltzmann/c-rbm-bernoulli/
    checkpoints/
      class_<k>/
        RBM_best.weights.h5
        RBM_last.weights.h5
...
You may override specific artifact dirs via:
  cfg['artifacts']['restrictedboltzmann']['c-rbm-bernoulli']['checkpoints' | 'summaries']

What this module provides
-------------------------
- build_visible_dataset(...)  → tf.data over flattened visible vectors (B, V) in {0,1}
- cd_k_update(...)            → one Contrastive Divergence (CD-k) update
- train_rbm(...)              → per-class training loop with early stopping + checkpoints
...
- This RBM is trained *per class* (K independent RBMs) to match the evaluator’s
  class-conditional synthetic generation contract used across model families.
...
def _coerce_cfg(cfg_or_argv) -> Dict[str, Any]:
    """
    Accept either:
      - a Python dict (already-parsed unified config), OR
    ...
    if isinstance(cfg_or_argv, (list, tuple)):
        ...
        p = argparse.ArgumentParser(description="RBM trainer (c-rbm-bernoulli)")
...
# ===========================
# Rule A: dataset-scoped artifact paths
# ===========================
@dataclass(frozen=True)
class ArtifactPaths:
    """
    Dataset-scoped, variant-scoped artifact directories for this RBM variant.

    Default layout:
      artifacts/<dataset_id>/restrictedboltzmann/c-rbm-bernoulli/{checkpoints,summaries}
    """
    root: Path
    checkpoints: Path
    summaries: Path
...
def _resolve_artifact_paths(cfg: Dict[str, Any]) -> ArtifactPaths:
    ...
    base = artifacts_root / dataset_id / "restrictedboltzmann" / "c-rbm-bernoulli"

    ckpt_override = _cfg_get(cfg, "artifacts.restrictedboltzmann.c-rbm-bernoulli.checkpoints", None)
    sum_override = _cfg_get(cfg, "artifacts.restrictedboltzmann.c-rbm-bernoulli.summaries", None)
...
def _load_dataset(cfg: Dict[str, Any], img_shape: Tuple[int, int, int], num_classes: int):
    ...
    # Split test into val + test (best-effort)
...
def _int_labels(y: np.ndarray, num_classes: int) -> np.ndarray:
    """Coerce one-hot labels → int labels; pass through if already int."""
...
def build_visible_dataset(
    ...
    *,
    ...
    ----------
    x : np.ndarray
        Images in shape (N,H,W,C) or flattened (N,V), float in [0,1] (or byte-like).
    binarize : bool
    ...
    # Normalize byte-like arrays to [0,1]
...
def cd_k_update(
    ...
    *,
    ...
    """
    One CD-k parameter update step.
    ...
    # Gradients of log-likelihood approximation
...
class RBMTrainConfig:
    """Strongly-typed knobs for RBM training."""
...
def train_rbm(
    ...
    *,
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        if np.isfinite(monitor) and (monitor < best_val - 1e-9):
...
def _run_train(cfg: Dict[str, Any]) -> int:
    ...
    Unified entrypoint:
      ...
      - Loads data
      - Trains per-class RBMs into checkpoints/class_<k>/
    ...
    # ---- Hyperparameters (RBM-specific keys with legacy fallbacks) ----
    ...
    lr = float(_cfg_get(cfg, "rbm.lr", _cfg_get(cfg, "RBM_LR", _cfg_get(cfg, "LR", 1e-3))))
    ...
    for k in range(K):
        ...
        # Optional class-specific val split
        x_val_k = None
    ...
    try:
        # If you have a variant-local preview helper, keep it optional.
...
def main(argv=None) -> int:
    ...
    if argv is None:
        ...
        p = argparse.ArgumentParser(description="Train RBM (c-rbm-bernoulli)")
        p.add_argument("--config", default="configs/config.yaml")
        args = p.parse_args()
