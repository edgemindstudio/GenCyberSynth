# src/gencysynth/models/restrictedboltzmann/variants/c-rbm-bernoulli/model.py
"""
Restricted Boltzmann Machine (RBM) model components.

Rule A note (Artifacts)
-----------------------
This file is **model-only**. It MUST NOT read/write artifacts directly.
...
Keeping model code artifact-free makes it reusable across:
  • multiple datasets
...
class RBMConfig:
    ...
    - `seed` is used by build_rbm() to seed TF/NumPy (best-effort reproducibility).
    """
    visible_units: int
...
def to_float01(x: np.ndarray) -> np.ndarray:
    ...
    Accepts either:
      ...
      - uint8/float arrays in [0,255] (auto-normalized)
    """
...
def flatten_images(
    ...
    *,
    ...
    ----------
    x : np.ndarray
        (N,H,W,C) or already-flattened (N,V).
    img_shape : (H,W,C)
        Target image shape used if x is 4D.
    assume_01 : bool
        If False, auto-normalize from 0..255 via to_float01.
...
class RBM(tf.keras.Model):
    ...
    def call(self, inputs: tf.Tensor, training: bool = False) -> tf.Tensor:
        """
        One-step reconstruction probabilities v -> h_prob -> v_prob.
    ...
    def train_step_cd(
        ...
        *,
        ...
        """
        One contrastive-divergence update (CD-k).
...
def build_rbm(cfg: RBMConfig) -> RBM:
    """
    Build an RBM from RBMConfig, seeding TF/NumPy (best-effort) and
    creating variables immediately so save/load works consistently.

    This stays artifact-free; it only constructs the model.
    """
    ...
    # Eager-build by calling once (ensures variables are initialized and
...
class BernoulliRBM(RBM):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Back-compat: allow visible_units/hidden_units in kwargs
